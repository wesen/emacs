...
;; RCS:		$Id: vhdl-mode.el,v 33.67 2008/11/27 19:13:09 reto Exp reto $
;; Keywords:	languages vhdl
;; WWW:		http://www.iis.ee.ethz.ch/~zimmi/emacs/vhdl-mode.html

(defconst vhdl-version "3.33.27"
  "VHDL Mode version number.")

(defconst vhdl-time-stamp "2008-11-27"
...
;; tested on: GNU Emacs 20.4/21.3/22.1, XEmacs 21.1 (marginally)
...
(defconst vhdl-begin-fwd-re
  "\\b\\(is\\|begin\\|block\\|component\\|generate\\|then\\|else\\|loop\\|process\\|procedural\\(\\s-+body\\)?\\|units\\|use\\|record\\|protected\\|for\\)\\b\\([^_]\\|\\'\\)"
		   ...
		   (and (vhdl-last-word (point))
			(or (vhdl-first-word (point))
			    (save-excursion
			      ...
			      (vhdl-backward-skip-label lim)
			      (vhdl-first-word (point)))))))
	  ;; "component", "units", "record", "protected":
	  ((looking-at "c\\|un\\|r\\|pr")
    ...
    (let (pos)
	  ...
	  (if (looking-at "el")
	    ...
	    (cond
	     ...
	     ((looking-at "\\(units\\|record\\|protected\\)\\b[^_]")
	      (vector (buffer-substring (match-beginning 1)
					(match-end 1))
		      (vhdl-first-word pos)
		      nil t))
...
(defconst vhdl-trailer-re
  "\\b\\(is\\|then\\|generate\\|loop\\|record\\|protected\\|use\\)\\b[^_]")
...
(defun vhdl-update-sensitivity-list ()
  "Update sensitivity list."
    (let ((proc-beg (point))
	  ...
	  (proc-mid (re-search-backward "^\\s-*\\(\\(begin\\)\\|process\\)\\>" nil t))
	  seq-region-list)
      (cond
